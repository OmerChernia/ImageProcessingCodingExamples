...
export default function MeanFilterPage() {
  ...
  const handleImageSelect = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        setError('Failed to compute histogram');
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/mean-filter', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleDownload = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `${originalName}_mean_${kernelSize}.png`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Mean Filter</h1>
