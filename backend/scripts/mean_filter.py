...
def apply_mean_filter(image, kernel_size):
    """Apply mean filter to the image with specified kernel size"""
...
def main():
    ...
    while True:
        ...
        # Process image
        try:
            processed_image = apply_mean_filter(image, kernel_size)
            # Save result
            output_path = f"{os.path.splitext(image_path)[0]}_mean_{kernel_size}.png"
